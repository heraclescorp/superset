    ...
    <Shell height={height}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ margin: '4px 0 0', fontSize: 13, color: '#9ca3af' }}>
          {fmtNum(stats.total)} total exposures · {stats.controlName}:{' '}
          {fmtNum(stats.controlTotal)}
          ...
          <span>
            ...
            {!stats.srm.passed && (
              <span style={{ color: '#b91c1c' }}>
                {' '}
                — Traffic split may be imbalanced
              </span>
            )}
          </span>
          <span style={{ color: '#6b7280', fontSize: 12 }}>
            Expected {fmt(stats.srm.expectedRatio)} · Observed{' '}
            {fmt(stats.srm.observedRatio)} · p = {fmtP(stats.srm.pValue)}
          </span>
        </div>
      )}
          ...
          {stats.variantMetrics.map((v, i) => (
            <button
              ...
              style={{
                ...
                border: `1px solid ${
                  selectedVariant === i ? '#6366f1' : '#e5e7eb'
                }`,
              }}
            >
                ...
                return (
                  <tr key={m.stage} style={{ background: rowBg }}>
                    ...
                    >
                      ...
                      >
                        ({m.relativeDelta > 0 ? '+' : ''}
                        {(m.relativeDelta * 100).toFixed(1)}% rel)
                      </div>
                    </td>
                    <td
                      style={{
                        ...
                        textAlign: 'right',
                        fontFamily: 'monospace',
                        fontWeight: m.pValue < 0.05 ? 700 : 400,
                      }}
                    >
                      {fmtP(m.pValue)}
                    </td>
                    ...
                    <td style={{ ...tdStyle, textAlign: 'center' }}>
                      ...
                      >
                        {m.direction.charAt(0).toUpperCase() +
                          m.direction.slice(1)}
                      </span>
                    </td>
                  </tr>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {variant.metrics.map((m, idx) => {
              const maxCount = Math.max(
                ...variant.metrics.map(s =>
                  Math.max(s.controlCount, s.variantCount),
                ),
              );
              const ctrlW =
                maxCount > 0 ? (m.controlCount / maxCount) * 100 : 0;
                  ...
                  >
                    ...
                    <span style={{ fontSize: 12, color: '#6b7280' }}>
                      <span style={{ color: '#3b82f6' }}>
                        {fmtNum(m.controlCount)}
                      </span>
                      {' vs '}
                      <span style={{ color: '#8b5cf6' }}>
                        {fmtNum(m.variantCount)}
                      </span>
                      {m.delta !== 0 && (
                        <span
                          style={{
      ...
      {rows.length === 0 && (
        <div style={{ textAlign: 'center', padding: 48, color: '#9ca3af' }}>
          ...
          <div>
            Navigate from the{' '}
            <a
              href="/superset/dashboard/experiments-v2/"
              style={{ color: '#6366f1' }}
            >
              experiment list
            </a>
            , or add an <strong>EXPERIMENT_SPEC_ID</strong> filter in the
            control panel.
          </div>
          <div style={{ marginTop: 8, fontSize: 12 }}>
            Use <strong>Save As</strong> to create a saved view for a specific
            experiment.
          </div>
        </div>
      )}
        ...
        <div style={{ textAlign: 'center', padding: 48, color: '#9ca3af' }}>
          {stats.groupNames.length < 2
            ? `Only one group found (${
                stats.groupNames[0] || '?'
              }). Need at least a control and variant to compare.`
